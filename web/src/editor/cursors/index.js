...
QuillCursors.prototype.registerTextChangeListener = function () {
  this.quill.on(this.quill.constructor.events.TEXT_CHANGE,
...
QuillCursors.prototype._initOptions = function (options) {
  ...
  this.options.autoRegisterListener = (options.autoRegisterListener === false)
    ? options.autoRegisterListener
    : this.options.autoRegisterListener
  this.options.hideDelay = (options.hideDelay === undefined)
    ? this.options.hideDelay
    : options.hideDelay
  this.options.hideSpeed = (options.hideSpeed === undefined)
...
QuillCursors.prototype._buildCursor = function (userId, name) {
  ...
  caretEl.querySelector(
    '.ql-cursor-caret').style.backgroundColor = cursor.color
...
QuillCursors.prototype._updateCursor = function (cursor) {
  ...
  if (!startLeaf || !endLeaf ||
    ...
    console.warn('[quill-cursors] A cursor couldn\'t be updated (ID ' +
      cursor.userId + '), hiding.')
  ...
  const _domtagName = _.get(startLeaf, '[0].domNode.tagName')
  if (_domtagName === 'IMG' || _domtagName === 'VIDEO') {
    range.setStart(startLeaf[0].domNode, 0)
    range.setEnd(endLeaf[0].domNode, 0)
  } else {
...
QuillCursors.prototype._updateCaret = function (cursor, leaf) {
  ...
  // by -1;
  if (index > 0 && leaf[1] === 0 && cursor.range.index !==
...
QuillCursors.prototype._updateSelection = function (
  cursor, rects, containerRect) {
